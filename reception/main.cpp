...
//#reference about sending image data over  tcp socket
//https://github.com/Dayof/opencv-socket/blob/master/server.cpp
//https://github.com/Dayof/opencv-socket/blob/master/client.cpp
//https://github.com/Avandrea/opencv-image-over-udp/blob/master/Example/main.cpp
...
//https://github.com/ultralytics/ultralytics/tree/7564930f1ad841ab9972712c5508c47c89480e54/examples/YOLOv8-LibTorch-CPP-Inference
...
void yolo_image_inference (const std::string& imageFileName);
void yolo_image_inference_local();
...
int main() {
    ...
    observer.incomingPacketHandler = yolo_inference;
    observer.incomingImageHandler = yolo_image_inference;
...
{
    while (1) {
        std::cout << std::endl << "please enter the full path of image file. " << std::endl;
        try {
            ...
            auto start_ms = std::chrono::time_point_cast<std::chrono::milliseconds>(std::chrono::system_clock::now());
            // Get the millisecond count
            uint32_t start = start_ms.time_since_epoch().count();
            ...
            torch::Tensor image_tensor = torch::from_blob(input_image.data, {input_image.rows, input_image.cols, 3},
                                                          torch::kByte).to(device);
            ...
            // Get the current time point and Convert to milliseconds since epoch
            auto end_ms = std::chrono::time_point_cast<std::chrono::milliseconds>(std::chrono::system_clock::now());
            // Get the millisecond count
            uint32_t end = end_ms.time_since_epoch().count();


            // Show the results
            std::cout << std::endl << "Inference takes [ " << end - start
                      << " ]ms,  results : [x1,y1,x2,y2], confidence level, class " << std::endl;
            for (int i = 0; i < keep.size(0); i++) {
                ...
                std::cout << "  Rect: [" << x1 << "," << y1 << "," << x2 << "," << y2 << "], Center: [" << (x2 + x1) / 2
                          << "," << (y2 + y1) / 2 << "]  Conf: " << conf
...
void yolo_image_inference (const std::string& imageFileName){
    ...
    cv::Mat image = cv::imread(imageFileName.c_str());
